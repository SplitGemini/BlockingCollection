...
struct ConditionVarTraits<std::condition_variable, std::mutex> {
    ...
    static void signal(std::condition_variable &cond_var) { cond_var.notify_one(); }

    static void broadcast(std::condition_variable &cond_var) { cond_var.notify_all(); }
    ...
    static bool wait_for(std::condition_variable &cond_var, std::unique_lock<std::mutex> &lock,
...
template <typename ThreadContainerType, typename SignalStrategyType, typename ConditionVarType,
          typename LockType>
class ConditionVariable {
public:
    ...
    /// upper-bound.
    ConditionVariable() :
        total_workers_(0), active_workers_(0), bounded_capacity_(SIZE_MAX), item_count_(0) {
    ...
    void signal() {
        ...
        if (signal_.should_signal(active_workers_, total_workers_, item_count_,
                                  bounded_capacity_)) {
            increment_active();
            ConditionVarTraits<ConditionVarType, LockType>::signal(condition_var_);
        }
    ...
    bool wait_for(std::unique_lock<LockType> &              lock,
        ...
        bool timed_out = ConditionVarTraits<ConditionVarType, LockType>::wait_for(condition_var_,
                                                                                  lock, rel_time);

        if (timed_out) { increment_active(); }
...
private:
    ...
    void decrement_total() { total_workers_ = total_workers_ > 0 ? total_workers_ - 1 : 0; }

    /// Increments the active worker count for this condition variable by 1.
    void increment_active() {
        if (++active_workers_ > total_workers_) active_workers_ = total_workers_;
    }

    /// Decrements the active worker count for this condition variable by 1.
    void decrement_active() { active_workers_ = active_workers_ > 0 ? active_workers_ - 1 : 0; }
...
struct NotEmptySignalStrategy {
    bool should_signal(size_t active_workers, size_t total_workers, size_t item_count,
                       size_t /*capacity*/) const {
        return active_workers == 0
               || (active_workers < total_workers && item_count / active_workers > ItemsPerThread);
    }
};
...
template <typename ThreadContainerType, typename NotFullSignalStrategy,
          typename NotEmptySignalStrategy, typename ConditionVarType, typename LockType>
struct ConditionVariableGenerator {
    using NotFullType =
        ConditionVariable<ThreadContainerType, NotFullSignalStrategy, ConditionVarType, LockType>;
    using NotEmptyType =
        ConditionVariable<ThreadContainerType, NotEmptySignalStrategy, ConditionVarType, LockType>;
...
public:
    ...
    bool add() {
        ...
        if (itr != thread_id_.end()) { return false; }
    ...
    /// Otherwise false.
    bool remove() {
        if (thread_id_.erase(ThreadContainerTraits<T>::get_thread_id()) > 0) { return true; }
        return false;
    }
...
public:
    ...
    Container() : bounded_capacity_(SIZE_MAX) {}

    /// Sets the max number of elements this container can hold.
    /// @param bounded_capacity The max number of elements this
    /// container can hold.
    void bounded_capacity(size_t bounded_capacity) { bounded_capacity_ = bounded_capacity; }
    ...
    /// Attempts fetch a value from index
    /// @param item When this method returns, if the element was
    /// fetched and returned successfully, item
    /// contains the fetched element. If no element was available
...
using StdConditionVariableGenerator =
    ConditionVariableGenerator<ThreadContainer<std::thread::id>, NotFullSignalStrategy<16>,
...
public:
    ...
    BlockingCollection() : BlockingCollection(SIZE_MAX) {}
    ...
    void complete_adding() {
        ...
        if (is_adding_completed_) return;
    ...
    template <
        typename U, class Rep, class Period,
    ...
    /// @param item at least one item give
    /// @param args arguments to forward to the constructor of the element
    /// @return A BlockCollectionStatus code.
    /// @see BlockingCollectionStatus
    template <typename... Args>
    ...
    BlockingCollectionStatus try_emplace_timed(const std::chrono::duration<Rep, Period> &rel_time,
                                               Args &&...args) {
        {
            ...
            if (BlockingCollectionStatus::Ok != status) return status;

            if (!container_.try_emplace(std::forward<Args>(args)...))
                return BlockingCollectionStatus::InternalError;

            // histoty item size increment
            history_added_size_ += sizeof...(Args);
            signal(container_.size(), false);
        }
        return BlockingCollectionStatus::Ok;
    }

    /// Fetch an item from the BlockingCollection<T>.
    /// A call to at may block until an item is available to be fetched.
    /// @param[out] item The item fetched from the collection.
    /// @param[in] index fetched item index of collection
    /// @return A BlockCollectionStatus code.
    /// @see BlockingCollectionStatus
    BlockingCollectionStatus at(T &item, std::size_t index) {
        return try_at(item, index, std::chrono::milliseconds(-1));
    }

    /// Fetch an item from the BlockingCollection<T>.
    /// If the collection is size is less or equal to index,
    /// this method immediately returns without fetch an item.
    /// @param[out] item The item fetched from the collection.
    /// @param[in] index fetched item index of collection
    /// @return A BlockCollectionStatus code.
    /// @see BlockingCollectionStatus
    BlockingCollectionStatus try_at(T &item, std::size_t index) {
        return try_at(item, index, std::chrono::milliseconds::zero());
    }

    /// Fetch an item from the BlockingCollection<T>.in the
    ...
    BlockingCollectionStatus try_at(T &item, std::size_t index,
                                    const std::chrono::duration<Rep, Period> &rel_time) {
        {
            ...
            auto status = wait_size_condition(guard, rel_time, index);

            if (BlockingCollectionStatus::Ok != status) return status;

            if (!container_.try_at(item, index)) return BlockingCollectionStatus::InternalError;

            // "at" do nothing write, ignore signal
            // signal(container_.size(), false);
        }
        return BlockingCollectionStatus::Ok;
    }
    ...
    /// @return A BlockCollectionStatus code.
    /// @see BlockingCollectionStatus
    BlockingCollectionStatus take(T &item) { return try_take(item, std::chrono::milliseconds(-1)); }
    ...
    /// @see http://en.cppreference.com/w/cpp/chrono/duration
    template <class Rep, class Period>
    BlockingCollectionStatus try_take(T &item, const std::chrono::duration<Rep, Period> &rel_time) {
        {
            std::unique_lock<LockType> guard(lock_);
            ...
            if (!container_.try_take(item)) return BlockingCollectionStatus::InternalError;
    ...
    BlockingCollectionStatus add_bulk(Iterator first, Iterator last, size_t &added) {
    ...
    /// @see BlockingCollectionStatus
    template <typename Iterator>
    BlockingCollectionStatus try_add_bulk(Iterator first, Iterator last, size_t &added) {
    ...
    BlockingCollectionStatus try_add_bulk(Iterator first, Iterator last, size_t &added,
                                          const std::chrono::duration<Rep, Period> &rel_time) {
        {
            ...
            if (first == last) return BlockingCollectionStatus::InvalidIterators;

            for (; first != last; ++first) {
                if (!container_.try_add((*first))) break;
                ++added;
                ++history_added_size_;
            }
    ...
    /// @see BlockingCollectionStatus
    template <typename Iterator>
    BlockingCollectionStatus take_bulk(Iterator first, size_t count, size_t &taken) {
    ...
    /// @see BlockingCollectionStatus
    template <typename Iterator>
    BlockingCollectionStatus try_take_bulk(Iterator first, size_t count, size_t &taken) {
        return try_take_bulk(first, count, taken, std::chrono::milliseconds::zero());
    }
    ...
    BlockingCollectionStatus try_take_bulk(Iterator first, size_t count, size_t &taken,
                                           const std::chrono::duration<Rep, Period> &rel_time) {
        {
            ...
            if (count == 0) return BlockingCollectionStatus::Ok;
            ...
            for (; first != end; ++first) {
                if (!container_.try_take((*first))) break;
...
private:
    class Iterator {
    public:
        Iterator(BlockingCollection<T, ContainerType, ConditionVariableGenerator> &collection) :
    ...
    private:
        BlockingCollection<T, ContainerType, ConditionVariableGenerator> &collection_;
...
private:
    ...
    /// @see is_full
    bool is_full_i() {
        return bounded_capacity_ != SIZE_MAX && container_.size() >= bounded_capacity_;
    }
...
protected:
    ...
    void signal(size_t itemCount, bool signal_not_full) {
        ...
        size_condition_var_.size(itemCount);

        if (signal_not_full) {
            // signal only if capacity is bounded
            if (bounded_capacity_ != SIZE_MAX) { not_full_condition_var_.signal(); }
        } else {
            not_empty_condition_var_.signal();
            size_condition_var_.signal();
        }
    }
    ...
    wait_not_full_condition(std::unique_lock<LockType> &              lock,
        ...
        if (is_adding_completed_i()) return BlockingCollectionStatus::AddingCompleted;
    ...
    wait_not_empty_condition(std::unique_lock<LockType> &              lock,
        ...
        if (is_completed_i()) return BlockingCollectionStatus::Completed;
    ...
    template <class Rep, class Period>
    BlockingCollectionStatus wait_size_condition(std::unique_lock<LockType> &              lock,
                                                 const std::chrono::duration<Rep, Period> &rel_time,
                                                 std::size_t                               index) {
        if (state_ == BlockingCollectionState::Deactivated)
            return BlockingCollectionStatus::NotActivated;
        ...
        // check index of element exists in container
        while (container_.size() <= index) {
            if (rel_time == std::chrono::duration<Rep, Period>::zero()) {
                status = BlockingCollectionStatus::TimedOut;
                break;
            }
            ...
            if (rel_time.count() < 0) {
                size_condition_var_.wait(lock);
            } else {
                if (size_condition_var_.wait_for(lock, rel_time)) {
                    status = BlockingCollectionStatus::TimedOut;
                    break;
                }
            }
...
private:
    ...
    // added item size from collection create
    size_t history_added_size_;
    ...
    // reuse not_empty ConditionVariableGenerator as size behavior similar with not_empty
    typename ConditionVariableGenerator::NotEmptyType size_condition_var_;
...
public:
    ...
    PriorityContainer() : bounded_capacity_(SIZE_MAX) {}
    ...
    bool try_emplace(Args &&...args) {
        ...
        if (container_.empty()) {
            ...
            typename container_type::reverse_iterator itr = std::find_if(
                container_.rbegin(), container_.rend(), [&new_item, this](value_type &item) {
    ...
    bool try_take_prio(value_type &item) {
        ...
        typename container_type::reverse_iterator itr = std::find_if_not(
            ...
            [&current_priority, &init_current_priority, this](value_type &item) -> bool {
                ...
                if (continue_search) { current_priority = &item; }
...
template <typename T, typename ContainerType = PriorityContainer<T, PriorityComparer<T>>,
          typename ConditionVariableGenerator = StdConditionVariableGenerator>
...
public:
    ...
    PriorityBlockingCollection() : base() {}

    /// Initializes a new instance of the PriorityBlockingCollection<T>
    /// class with the specified upper-bound.
    /// @param capacity The bounded size of the collection.
    explicit PriorityBlockingCollection(size_t capacity) : base(capacity) {}
    ...
    /// collection.
    void take_prio(T &item) { try_take_prio(item, std::chrono::milliseconds(-1)); }
    ...
    BlockingCollectionStatus try_take_prio(T &                                       item,
                                           const std::chrono::duration<Rep, Period> &rel_time) {
        {
            std::unique_lock<typename ConditionVariableGenerator::lock_type> guard(base::lock());
    ...
    /// @see BlockingCollectionStatus
    template <typename Iterator>
    BlockingCollectionStatus take_prio_bulk(Iterator first, size_t count, size_t &taken) {
        return try_take_prio_bulk(first, count, taken, std::chrono::milliseconds(-1));
    }
    ...
    /// @see BlockingCollectionStatus
    template <typename Iterator>
    BlockingCollectionStatus try_take_prio_bulk(Iterator first, size_t count, size_t &taken) {
        return try_take_prio_bulk(first, count, taken, std::chrono::milliseconds::zero());
    }
    ...
    try_take_prio_bulk(Iterator first, size_t count, size_t &taken,
                       const std::chrono::duration<Rep, Period> &rel_time) {
        {
            ...
            for (; first != end; ++first) {
                if (!base::container().try_take_prio((*first))) break;
...
class Guard {
public:
    explicit Guard(BlockingCollectionType &collection) : collection_(collection) {
...
/// BlockingCollection.
template <typename T>
using BlockingPriorityQueue = BlockingCollection<T, PriorityContainer<T, PriorityComparer<T>>>;
...
struct ConditionVarTraits<CONDITION_VARIABLE, WIN32_SRWLOCK> {
    static void initialize(CONDITION_VARIABLE &cond_var) { InitializeConditionVariable(&cond_var); }

    static void signal(CONDITION_VARIABLE &cond_var) { WakeConditionVariable(&cond_var); }

    static void broadcast(CONDITION_VARIABLE &cond_var) { WakeAllConditionVariable(&cond_var); }
    ...
    template <class Rep, class Period>
    static bool wait_for(CONDITION_VARIABLE &cond_var, std::unique_lock<WIN32_SRWLOCK> &lock,
                         const std::chrono::duration<Rep, Period> &rel_time) {
        DWORD milliseconds = static_cast<DWORD>(rel_time.count());

        if (!SleepConditionVariableSRW(&cond_var, &lock.mutex()->native_handle(), milliseconds,
                                       0)) {
            if (GetLastError() == ERROR_TIMEOUT) return true;
        }
...
struct ConditionVarTraits<CONDITION_VARIABLE, WIN32_CRITICAL_SECTION> {
    ...
    static void wait(CONDITION_VARIABLE &cond_var, std::unique_lock<WIN32_CRITICAL_SECTION> &lock) {
        SleepConditionVariableCS(&cond_var, &lock.mutex()->native_handle(), INFINITE);
    }
    ...
    static bool wait_for(CONDITION_VARIABLE &                      cond_var,
        ...
        if (!SleepConditionVariableCS(&cond_var, &lock.mutex()->native_handle(), milliseconds)) {
            if (GetLastError() == ERROR_TIMEOUT) return true;
        }
